...
export default function SignalAlertCard({
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl border p-6 shadow-lg transition-all duration-300 hover:shadow-2xl ${
      status === 'resolved' 
        ? 'border-green-950 bg-zinc-950/40 opacity-70 hover:opacity-100 hover:border-green-900/60' 
        : 'border-zinc-800/80 bg-zinc-950 hover:border-zinc-700'
    }`}>
      
      {/* Risk Score Glow Effect */}
      <div className={`absolute top-0 right-0 -mr-6 -mt-6 h-24 w-24 rounded-full blur-2xl transition-opacity duration-300 group-hover:opacity-100 ${
        status === 'resolved' ? 'bg-green-500/5' : 'bg-red-500/10'
      }`} />
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className={`inline-flex h-7 items-center justify-center rounded-lg px-2.5 text-xs font-bold tracking-wider ${
            status === 'resolved' ? 'bg-zinc-800 text-zinc-400' :
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-zinc-900 border border-zinc-800/50">
            ...
            <div className={`absolute top-0 right-0 h-2.5 w-2.5 rounded-full ring-2 ring-zinc-950 ${
              status === 'resolved' ? 'bg-green-500' : severity.indicator
            }`} />
          </div>
        </div>
      </div>
      ...
      {/* Title */}
      <h3 className={`mt-4 text-base font-semibold leading-snug transition-colors duration-200 ${
        status === 'resolved' 
          ? 'text-zinc-400 line-through group-hover:text-zinc-300' 
      ...
      <div className="mt-6 flex items-center justify-between border-t border-zinc-900 pt-4">
        {status === 'resolved' ? (
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-green-400 bg-green-500/10 rounded-full px-2.5 py-0.5">
        ...
        <div className="flex items-center gap-3">
          {onResolve && (
            <button
              ...
              className="text-xs text-zinc-400 hover:text-zinc-200 transition-colors duration-200 underline decoration-dotted bg-transparent border-0 cursor-pointer"
            >
